...
class DetectOpenWater(ALSPointCloudFilter):
    ...
    def apply(self, als, do_plot=False, savefig=False):
        ...
        mask_roll = np.where(np.isfinite(als.get('aircraft_roll')))
        # Indexes of all NADIR pixels
        nadir_inds = (mask_roll[0], (
                    np.ones(mask_roll[0].size) * als.n_shots / 2 + als.get('aircraft_roll')[mask_roll] / self.cfg[
                "fov_resolution"]).astype('int'))
        ...
        if np.where(elev_nadir == globmin)[0].size > 0:
            ...
            elev_tol = self.cfg["elev_tol"]
            elev_grad = self.cfg["elev_segment"] / als.n_lines
            owp = np.arange(elev_nadir.size)[np.abs(elev_nadir - globmin) <= np.abs(
                np.arange(elev_nadir.size) - iglobmin) * elev_grad + 2 * elev_tol]
            ...
            if do_plot:
                ...
                ax[1, 0].fill_between(np.arange(als.n_lines)[mask_roll][mask_ex_nadir],
                                      (np.abs(
                                          np.arange(elev_nadir.size) - iglobmin) * elev_grad + globmin + 2 * elev_tol),
                                      (np.abs(np.arange(elev_nadir.size) - iglobmin) * -(
                                          elev_grad) + globmin - 2 * elev_tol), color='0.9')
                ...
                try:
                    ...
                    ax[1, 0].set_xticklabels(
                        ['%i' % np.round(time_nadir[int(indt)] - time_nadir[0]) for indt in ax[1, 0].get_xticks()])
                except:
                    ax[1, 0].set_xlabel('Line No.')

                ax[1, 1].plot(np.arange(als.n_lines)[mask_roll][mask_ex_nadir],
                              np.ones(elev_nadir.size) * self.cfg["rflc_thres"] + np.nanmean(rflc_nadir_m), '--',
                              color='0.7')
                ax[1, 1].plot(np.arange(als.n_lines)[mask_roll][mask_ex_nadir],
                              -np.ones(elev_nadir.size) * self.cfg["rflc_thres"] + np.nanmean(rflc_nadir_m), '--',
                              color='0.7')
                ...
                try:
                    ...
                    ax[1, 1].set_xticklabels(
                        ['%i' % np.round(time_nadir[int(indt)] - time_nadir[0]) for indt in ax[1, 1].get_xticks()])
                except:
            ...
            for i in cluster_breaks:
                if ind_start != i:
                    ...
                    if self.proj_avail:
                        ...
                        x, y = np.zeros(als.get('longitude').shape) * np.nan, np.zeros(
                            als.get('longitude').shape) * np.nan
            ...
            if do_plot:
                ind_start = 0
                for i in cluster_breaks:
                    ax[0, 0].scatter(nadir_inds[0][owp[ind_start:i]], nadir_inds[1][owp[ind_start:i]], c='0.85',
                                     edgecolors='0.4', zorder=10)
                    ax[0, 1].scatter(nadir_inds[0][owp[ind_start:i]], nadir_inds[1][owp[ind_start:i]], c='0.85',
                                     edgecolors='0.4', zorder=10)
                    ax[1, 0].plot(np.arange(als.n_lines)[mask_roll][mask_ex_nadir][owp[ind_start:i]],
                                  elev_nadir[owp[ind_start:i]], ".")
                    ax[1, 1].plot(np.arange(als.n_lines)[mask_roll][mask_ex_nadir][owp[ind_start:i]],
                                  rflc_nadir[owp[ind_start:i]], ".")
                    ind_start = i + 1

                if savefig:
                    fig.savefig(Path(self.cfg["export_file"]).absolute().parent.joinpath(
                        als.tcs_segment_datetime.strftime('Open_water_detection_%Y%m%dT%H%M%S.jpg')),
                                dpi=300)
                    logger.info('OWDETC: Stored open water detection image to %s' % Path(
                        self.cfg["export_file"]).absolute().parent.joinpath(
                        'Open_water_detection_%s.jpg' % als.tcs_segment_datetime))
        ...
        for ix, iy in zip(inds_peak[0], inds_peak[1]):
            with self._get_export() as export_file:
                export_file.write('%.15f,%.15f,%.15f,%f,%f,%f\n' % (als.get('timestamp')[ix, iy],
                                                                    ...
                                                                    [np.zeros(als.get('longitude').shape) * np.nan,
                                                                     als.x][self.proj_avail][ix, iy],
                                                                    [np.zeros(als.get('longitude').shape) * np.nan,
                                                                     als.y][self.proj_avail][ix, iy],
    ...
    def freeboard_computation(self, als, dem_cfg=None):
        ...
        if self.cfg['SeaSurfaceInterpolation']['interp2d']:  # Use 2d interpolation of open water points
            ...
            if np.any([np.isnan(self.xow), np.isnan(self.yow)]):
                ...
                logger.info(
                    'FBCONV: WARNING: config projection is used to compute freeboard positions, which potentially interferes with Ice Drift Correction')
        ...
        if self.limit_freeboard:
            logger.info(
                'FBCONV: Freeboard conversion: limit the freeboard by lower envelope and maximum freeboard is activated')
            ...
            self.low_env = np.zeros((int(np.ceil(self.x.shape[0] / self.di) * 2 - 1),
                                     # np.floor(self.x.shape[0]/self.di*2-1).astype('int'),
                                     int(np.ceil(self.x.shape[
                                                     1] / self.dj) * 2 - 1)))  # np.floor(self.x.shape[1]/self.dj*2-1).astype('int')))
            ...
            self.low_env_xy = RBFInterpolator(
                np.rollaxis(np.stack([self.x_env[mask_low_env].flatten(), self.y_env[mask_low_env].flatten()]), 1, 0),
                self.low_env[mask_low_env].flatten(), kernel='linear')(
                np.rollaxis(np.stack([self.x.flatten(), self.y.flatten()]), 1, 0)).reshape(self.x.shape)
            ...
            self.mask_below_ssh = np.clip(1 - ((self.low_env_xy - self.ssh_int - self.min_fb_ice) / self.ramp_height),
                                          0, 1)  # ramped mask
            ...
            self.mask_above_ice = np.clip(
                1 - ((self.max_fb_ice - (self.low_env_xy - self.ssh.copy())) / self.ramp_height), 0, 1)  # ramped mask

            self.mask_max = self.mask_above_ice * self.dis
            if np.any(self.mask_max > 0): logger.info(
                'FBCONV: Lower envelope above maximum freeboard, correction required')
        ...
        try:
            ...
            if elev_cor_file.is_file():
                ...
                for iowp in np.unique(self.id_nearest_owp[np.isfinite(self.id_nearest_owp)]).astype('int'):
                    ...
                    fb_uncertainty[self.id_nearest_owp == iowp] += np.abs(
                        elev_cor[self.id_nearest_owp == iowp] - elev_cor_owp)
